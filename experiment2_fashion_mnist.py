...
import experiment_fashion_mnist
import glob
...
def showimg(imgs,ys,original_img=None):
    index=0
    for img,y in zip(imgs*0.5+0.5,ys):
...
def train_combined_model(train_x, train_y, test_x, test_y):
    ...
    epoch = 100
    iteration = 40
    model = experiment_fashion_mnist.BIGVAE().model()
    model.summary()
    ...
    for i in range(epoch):
        ...
        for j in range(iteration):
            with tf.GradientTape() as g:
                ...
                for k in range(2):
                    y, feature = model(test_x[k * 5000:k * 5000 + 5000])
                    y_pre = np.append(y_pre, y, axis=0)
                y_pre = np.delete(y_pre, 0, axis=0)
                acc_ = get_acc(y_pre, test_y)
                if acc < acc_ and acc_ > 0.9:
                    print('模型准确度从%.4f上升到%.4f' % (acc, acc_), end=',')
                    acc = acc_
                    model.save('model/fashion_mnist_combine_%.4f.h5' % (acc),overwrite=True)
                    print('模型保存成功。')
                    if acc_==1:
                        print('以获得最优模型，结束程序')
                        break
    ...
    showimg(feature.numpy().reshape(-1, 28, 28),y,test_x[index])
...
def evaluate_model(test_x, test_y):
    path_list = glob.glob('model/fashion_mnist_combine_0.9190.h5')
...
class MyCallBack(keras.callbacks.Callback):
    def on_batch_end(self, batch, logs=None):
        ...
        loss = logs.get('loss')
        c = str(acc) + ',' + str(loss) + '\n'
        with open('record/F_mnist_traing_noSP.csv', 'a') as f:
            f.write(c)
    def on_epoch_end(self, epoch, logs=None):
        val_acc = logs.get('val_accuracy')
        val_loss = logs.get('val_loss')
        c =  str(val_acc) + ',' + str(val_loss) + '\n'
        with open('record/F_mnist_test_noSP.csv', 'a') as f:
            f.write(c)
...
if __name__ == '__main__':
    ...
    # #训练分类器
    # cloud_model=experiment_fashion_mnist.Classifier().model()
    # opt = keras.optimizers.Adagrad(lr=0.0002)
    # SCC = keras.losses.SparseCategoricalCrossentropy()
    # cloud_model.compile(optimizer=opt, loss=SCC, metrics=['accuracy'])
    # cp_callback = tf.keras.callbacks.ModelCheckpoint(filepath='model/fashion_mnist_classifier.h5',
    #                                                  save_best_only=True,
    #                                                  mode='max',
    ...
    # 训练组合器
    cloud_model = experiment_fashion_mnist.BIGVAE().model()
    opt = keras.optimizers.Adagrad(lr=0.0002)
    SCC = keras.losses.SparseCategoricalCrossentropy()
    cloud_model.compile(optimizer=opt, loss=SCC, metrics=['accuracy'])
    cp_callback = tf.keras.callbacks.ModelCheckpoint(filepath='model/fashion_mnist_SP.h5',
                                                     save_best_only=True,
                                                     mode='max',
                                                     monitor='val_accuracy',
                                                     save_weights_only=False,
    ...
    # test_optimal(test_x, test_y,model_name='fashion_mnist_combine_0.9190.h5')
