...
if __name__ == '__main__':
    ...
    # # 训练分类器
    # cloud_model = keras.applications.inception_resnet_v2.InceptionResNetV2(include_top=True,weights=None,input_shape=(32,32,3))
    #
    # opt = keras.optimizers.Adam(lr=0.0002)
    # SCC = keras.losses.SparseCategoricalCrossentropy()
    # cloud_model.compile(optimizer=opt, loss=SCC, metrics=['accuracy'])
    # cp_callback = tf.keras.callbacks.ModelCheckpoint(filepath='model/cifar10_classifier_inception_resnet_v2_{val_accuracy:.4f}.h5',
    #                                                  save_best_only=True,
    #                                                  mode='max',
